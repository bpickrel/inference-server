...
sys.path.append('/workspace/proteus/examples/python/migraphx/more')
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            root, "examples/python/migraphx/more/utilities/yolov4.onnx"
        ),
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default=os.path.join(root, "examples/python/migraphx/more/utilities/coco.names"),
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default=os.path.join(root, "examples/python/migraphx/more/utilities/input.jpg"),  # an outdoor crowd scene
...
def main(args):
    ...
    # Load a picture.  Hardcoded shape from the yolo model
    # shape=[1, 416, 416, 3]
    ...
    #
    # create an image inference request and send it
    #
    images = [img]
    ...
    # detections = [np.array(out.getFp32Data(), dtype=float).reshape(out.shape) for out in response.getOutputs()]
    # detections = [np.ndarray(shape=out.shape, buffer=np.array(out.getFp32Data()), dtype=float) for out in response.getOutputs()]   

    #
    # Post-process the model outputs and display image with detection bounding boxes
    #  
    ANCHORS = "examples/python/migraphx/more/utilities/yolov4_anchors.txt"
    STRIDES = [8, 16, 32]
    ...
    image = ip.draw_bbox(original_image, bboxes, 'examples/python/migraphx/more/utilities/coco.names')

    image = Image.fromarray(image)
    output_name = "examples/python/migraphx/more/utilities/yolo4_output.jpg"
    image.save(output_name)

    print("Done.  Your marked-up image is at " + output_name)
